...
function convertSecondsToTime(seconds) {
  ...
  function findIndexesLessThanOrEqualTo(volumes, limit, segmentBreakLength) {
    ...
    for (let i = 0; i < volumes.length; i++) {
      ...
      //console.log(`${i} = ${convertSecondsToTime(i)} : ${volumes[i]} | no Δ`);
      ...
      if (volumes[i] <= limit) {
        ...
        if (delta >= segmentBreakLength && i !== 0) {
            console.log(`   [track #${gapCount} begins at above time] Δ >= ${segmentBreakLength}, gap ${gapCount}`)
            gapCount=gapCount+1;
...
//output2.log
//findIndexesLessThanOrEqualTo(volumes_input1_full, limit = -20, segmentBreakLength=70)
...
var volumes_input2_full = []

//try1
findIndexesLessThanOrEqualTo(
  volumes_input2_full,          //input list of volumes in db
  limit = -20,                  //volume in db used to determine when gaps occur
  segmentBreakLength=70         //if Δ delta is greater than or equal to segmentBreakLength, consider it a new track
)
